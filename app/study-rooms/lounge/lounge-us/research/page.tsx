...
import React, { useState } from "react"; // Removi useEffect pois não animamos mais os logs
...
import { 
    ...
    ServerStackIcon,
    CurrencyDollarIcon,
    LockClosedIcon, // Novo icone para o estado offline
    ExclamationTriangleIcon // Novo icone para o aviso
...
const RESEARCH_DATA: ResearchItem[] = Array.from({ length: 35 }).map((_, i) => {
    ...
    if (i === 0) {
        ...
        // Gera número aleatório entre 5 e 50
        progress = Math.floor(Math.random() * 46) + 5; 
    }

    let phds = 0;
    let masters = 0;
    
    // Mantendo quotas de graus
    if (i < 4) {
    ...
    return {
        ...
        progress: progress,
        // Status atualizado conforme o progresso reduzido
...
// --- SUB-COMPONENT: RESEARCH CARD (Intact) ---
...
function ResearchCard({ item }: ResearchCardProps) {
    ...
    const [ipStatus, setIpStatus] = useState<string>("Negotiate IP");

    const handleNegotiate = (e: React.MouseEvent) => {
        e.stopPropagation(); 
        setIpStatus("Not available yet");
    };
                        ...
                        <div className="mt-6 pt-4 border-t border-slate-200 dark:border-white/5 pl-2">
                            ...
                            <div className="mb-4">
                                <h5 className="text-[10px] font-bold uppercase text-slate-400 mb-2 flex items-center gap-2">
                                    <ServerStackIcon className="w-3 h-3" /> Agents in Production
                                </h5>
                            ...
                            <div className="grid grid-cols-2 gap-x-6 gap-y-3 mb-6">
                                {[
                                    { label: "Commercial App", val: item.details.commercial },
                                    { label: "Research App", val: item.details.research },
                                    { label: "Social Impact", val: item.details.social },
...
export default function ResearchModule() {
    ...
    // Removi o state e o useEffect dos logs, pois o chat está "trancado"

    return (
        <div className="w-full h-full flex flex-col gap-6">
            
            {/* --- TOP: AGENT HUB (OFFLINE STATE) --- */}
            <div className="w-full p-6 rounded-3xl bg-[#0f172a] border border-white/10 shadow-xl overflow-hidden relative group">
                ...
                <div className="relative z-10">
                    <div className="flex items-center gap-3 mb-4">
                        {/* Red light for offline */}
                        <div className="w-2 h-2 rounded-full bg-green-500" />
                        <h3 className="text-xs font-black uppercase tracking-[0.2em] text-white/90">35 ACTIVE PROJECTS</h3>
                    </div>
